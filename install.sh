#!/usr/bin/env bash
# Quint Skills installer (macOS / Linux)
#
# Usage:
...
#
# Environment:
#   QS_REPO_URL  Override clone URL (default: official GitHub repo)
#   QS_DIR       Override clone destination (default: $HOME/.quint-llm-kit/repo)
...
link_skills() {
  ...
  case "$style" in
    per-skill)
      ...
      ;;
    folder)
      ln -sfn "$root" "$target/quint-llm-kit"
      printf '  ✓ %s → %s\n' "$target/quint-llm-kit" "$root"
      ;;
    *)
  ...
  case "$style" in
    per-skill)
      if [[ -d "$(skills_root)" ]]; then
        ...
        # our skills tree so we can still clean up.
        local link resolved
...
Environment:
  QS_REPO_URL  Override clone URL (default: official repo)
  QS_DIR       Override clone destination (default: \$HOME/.quint-llm-kit/repo)
USAGE
}

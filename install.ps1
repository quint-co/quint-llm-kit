<#
.SYNOPSIS
  Quint Skills installer for Windows (PowerShell).
...
function Show-Usage {
    @"
Quint Skills installer (Windows)
...
Environment:
  QS_REPO_URL   Override clone URL
  QS_DIR        Override clone destination (default: %USERPROFILE%\.quint-llm-kit\repo)
"@
}
...
function Remove-Reparse([string]$Path) {
    ...
    # real files or directories so an existing user folder is never destroyed.
    if (-not (Test-Path $Path)) { return $false }
    $item = Get-Item -LiteralPath $Path -Force
    ...
    Write-Warning "Refusing to delete $Path — it is a real file/directory, not a junction/symlink we created. Remove it manually if intended."
    return $false
}
...
function Link-Skills([string]$Target, [string]$Style) {
    ...
    switch ($Style) {
        ...
        'folder' {
            $link = Join-Path $Target 'quint-llm-kit'
...
function Unlink-Skills([string]$Target, [string]$Style) {
    ...
    switch ($Style) {
        ...
        'folder' {
            Remove-Reparse (Join-Path $Target 'quint-llm-kit') | Out-Null
        }
    }
}
